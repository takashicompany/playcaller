...
```sh
claude mcp add playcaller -- sh -c "uvx --from 'mcp[cli]>=1.2.0' mcp run Library/PackageCache/company.takashi.playcaller@*/Server~/server.py"
```
...
claude mcp add playcaller \
  ...
  -- sh -c 'uvx --from "mcp[cli]>=1.2.0" mcp run "$UNITY_PROJECT_DIR/Library/PackageCache/company.takashi.playcaller@*/Server~/server.py"'
```
